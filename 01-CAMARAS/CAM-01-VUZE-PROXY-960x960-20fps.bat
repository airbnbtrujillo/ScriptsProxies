...
REM ================== CONFIG ==================
set "TEMP_DIR=Proxy VUZE RAW"
set "PROXY_SUFFIX= VuzeRawProxy.mp4"
set "TOL_SEC=5"          REM tolerancia de duraciÃ³n (seg)
set "CHECK_DIM=1"        REM 1=verificar 960x960 leyendo ffmpeg -i
set "V_FPS=20"
set "STRICT_COUNT=0"     REM 1=exige PROXY_COUNT==TOTAL siempre; 0=permite concat si falta el final
...
set "A_AR=16000"
set "A_CH=1"
set "VF=scale=960:960:force_original_aspect_ratio=decrease,pad=960:960:(ow-iw)/2:(oh-ih)/2,fps=%V_FPS%"
...
set "OUT_FILE=%PROJECT% VUZE RAW PROXY Complete.mp4"
set "OUT_FULL=%OUT_DIR%\%OUT_FILE%"
set "SIG_FILE=%OUT_DIR%\_concat_sig.txt"
set "STATUS_LOG=%OUT_DIR%\_keeper_status.txt"
set "FFMPEG_LOG=%OUT_DIR%\_concat_lastlog.txt"
...
call :LOG "==== VUZE Proxy Keeper (overlay N - NombreDelClip) ===="
...
REM ================== PASO 4: DECIDIR Y CONCATENAR ==================
set "NEED_CONCAT=0"
set "REASON="
...
:VALID
REM %1=orig %2=proxy -> %3=1 si valido (duracion Â±TOL y 960x960 opcional)
set "%~3=0"
...
if %CHECK_DIM%==1 call :HAS960 "%~2" D960 & if /I not "!D960!"=="1" goto :EOF
set "%~3=1"
goto :EOF
...
:HAS960
REM %1=path %2=ret -> 1 si aparece "960x960" en cualquier linea del -i
set "%~2="
set "_T=%TEMP%\ffinf_%RANDOM%%RANDOM%.txt"
"%FFMPEG%" -hide_banner -i "%~1" 1>nul 2>"%_T%"
for /f "usebackq delims=" %%L in ("%_T%") do (
