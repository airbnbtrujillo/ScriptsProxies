...
REM ================== CONFIG ==================
set "TEMP_DIR=Proxy GOPRO RAW"
set "PROXY_SUFFIX= GOPROProxy.mp4"
set "DIM_EXPECT=1080x608"
set "V_FPS=30"
set "TOL_SEC=5"
set "CHECK_DIM=1"
set "STRICT_COUNT=0"
set "RECURSIVE=1"
set "EXTS=mp4 mov m4v mkv avi"
set "STRIP_SUFFIX_LABEL=0"   REM 1=remover sufijo del texto mostrado, 0=mantener
...
REM --- filtro de proxy y expectativa de resolucion ---
set "VF=scale=1080:608:force_original_aspect_ratio=decrease,pad=1080:608:(ow-iw)/2:(oh-ih)/2,fps=%V_FPS%"

REM ================== OUT (carpeta padre) ==================
for %%I in ("%CD%\..") do (
  set "OUT_DIR=%%~fI"
  set "PROJECT=%%~nxI"
)
...
set "OUT_FILE=%PROJECT% GOPRO RAW PROXY Complete.mp4"
set "OUT_FULL=%OUT_DIR%\%OUT_FILE%"
set "SIG_FILE=%OUT_DIR%\_concat_sig_gopro.txt"
set "STATUS_LOG=%OUT_DIR%\_keeper_status_gopro.txt"
set "FFMPEG_LOG=%OUT_DIR%\_concat_lastlog_gopro.txt"

REM --- overlay assets ---
set "FILELIST=%CD%\filelist.txt"
set "ASS=%CD%\overlay.ass"
set "PS_ASS=%CD%\_mk_overlay_ass.ps1"
set "OVERLAY_LOG=%OUT_DIR%\_overlay_build.log"
...
if exist "%OVERLAY_LOG%" del /q "%OVERLAY_LOG%" >nul 2>&1

call :LOG "==== Proxy Keeper + Overlay (auto-close) ===="
call :LOG "[DBG] CWD=""%CD%"""
call :LOG "[DBG] OUT_FULL=""%OUT_FULL%"""
if exist "%OUT_FULL%" (call :LOG "[DBG] Final existe: SI") else (call :LOG "[DBG] Final existe: NO")
if not exist "%TEMP_DIR%" mkdir "%TEMP_DIR%"

REM ================== PASO 1: ESCANEAR Y PROCESAR ORIGINALES ==================
...
if "%RECURSIVE%"=="1" (
  for %%E in (%EXTS%) do for /r %%F in (*.%%E) do call :CONSIDER "%%~fF"
) else (
  for %%E in (%EXTS%) do for %%F in (*.%%E) do call :CONSIDER "%%~fF"
)
...
REM ================== PASO 2: RECONSTRUIR FILELIST SI FALTARA ==================
set "PROXY_COUNT=0"
if exist "%FILELIST%" for /f %%# in ('find /v /c "" ^< "%FILELIST%"') do set "PROXY_COUNT=%%#"
if %PROXY_COUNT% LSS %OK% (
  call :LOG "[FIX] Reconstruyendo filelist desde %TEMP_DIR%..."
  if exist "%FILELIST%" del /q "%FILELIST%"
  for /f "usebackq delims=" %%P in (`dir /b /a-d "%TEMP_DIR%\*%PROXY_SUFFIX%" 2^>nul ^| sort`) do (
    call :ADD_TO_LIST "%TEMP_DIR%\%%P"
  )
  set "PROXY_COUNT=0"
  if exist "%FILELIST%" for /f %%# in ('find /v /c "" ^< "%FILELIST%"') do set "PROXY_COUNT=%%#"
)
call :LOG "[INFO] filelist: %PROXY_COUNT% entradas"
if %PROXY_COUNT% EQU 0 (call :LOG "[ERROR] No hay proxies listados. Nada que concatenar." & goto :END)

REM ================== PASO 3: FIRMA DEL SET DE PROXIES ==================
set "_SIGSRC=%TEMP%\sigsrc_%RANDOM%%RANDOM%.txt"
if exist "%_SIGSRC%" del /q "%_SIGSRC%"
for /f "usebackq delims=" %%P in (`dir /b /a-d "%TEMP_DIR%\*%PROXY_SUFFIX%" 2^>nul ^| sort`) do (
  for %%Z in ("%TEMP_DIR%\%%P") do >>"%_SIGSRC%" echo %%~zZ;%%~tZ;%%~nP
)
...
if %PROXY_COUNT% NEQ %TOTAL% (
  ...
  if "%STRICT_COUNT%"=="1" (
    ...
    if exist "%OUT_FULL%" (set "COUNT_OK=0") else (set "COUNT_OK=1")
  )
)
call :LOG "[DBG] NEED_CONCAT=%NEED_CONCAT%  REASON=%REASON%  COUNT_OK=%COUNT_OK%"

if "%COUNT_OK%"=="0" goto :END
if "%NEED_CONCAT%"=="0" (call :LOG "[INFO] Sin cambios y final existe. Skip concat." & goto :END)

REM ================== PASO 5: GENERAR OVERLAY .ASS (titulo = nombre del proxy) ==================
if exist "%ASS%" del /q "%ASS%"
if exist "%PS_ASS%" del /q "%PS_ASS%"

set "SUFFIX_NOEXT=%PROXY_SUFFIX:.mp4=%"
if "%STRIP_SUFFIX_LABEL%"=="1" (set "ASS_SUFFIX=%SUFFIX_NOEXT%") else (set "ASS_SUFFIX=")

call :WRITE_PS_ASS "%PS_ASS%"
if errorlevel 1 (call :LOG "[ERROR] No se pudo crear %PS_ASS%." & goto :END)

call :LOG "[ASS ] Generando overlay.assâ€¦"
powershell -NoProfile -ExecutionPolicy Bypass -File "%PS_ASS%" ^
  -ListPath "%FILELIST%" -AssPath "%ASS%" -FFProbe "%FFPROBE%" -Suffix "%ASS_SUFFIX%" 1>>"%OVERLAY_LOG%" 2>>&1

if not exist "%ASS%" (call :LOG "[ERROR] No se pudo crear overlay.ass" & goto :END)
call :LOG "[OK  ] overlay.ass creado."

REM Escapar ruta ASS para filtro subtitles
set "ASS_ESC=%ASS:\=\\%"
set "ASS_ESC=%ASS_ESC::=\:%"

REM ================== PASO 6: CONCAT + OVERLAY ==================
...
%FFMPEG% -y -hide_banner -loglevel warning -stats -f concat -safe 0 -i "%FILELIST%" ^
  -vf "subtitles='%ASS_ESC%'" ^
  -c:v %V_CODEC% -rc vbr_hq -cq %V_CQ% -b:v %V_BV% -maxrate %V_MAX% -bufsize %V_MAX% -pix_fmt yuv420p -preset %V_PRESET% ^
  -c:a aac -b:a 96k -ar 48000 -ac 2 "%OUT_FULL%" >"%FFMPEG_LOG%" 2>&1

if errorlevel 1 (
  call :LOG "[WARN] Reintento con recodificacion (mismo overlay)..."
  %FFMPEG% -y -hide_banner -loglevel warning -stats -f concat -safe 0 -i "%FILELIST%" ^
    -vf "subtitles='%ASS_ESC%'" ^
    -c:v %V_CODEC% -rc vbr_hq -cq %V_CQ% -b:v %V_BV% -maxrate %V_MAX% -bufsize %V_MAX% -pix_fmt yuv420p -preset %V_PRESET% ^
    -c:a aac -b:a 96k -ar 48000 -ac 2 "%OUT_FULL%" >>"%FFMPEG_LOG%" 2>&1 || (
...
:CONSIDER
REM %1 = ruta candidata
set "P=%~1"

REM excluir carpeta de proxies
set "CHK=!P:%CD%\%TEMP_DIR%\=!"
if /I not "!CHK!"=="!P!" goto :EOF

REM excluir final (por si cae en misma ruta)
if /I "!P!"=="%OUT_FULL%" goto :EOF

REM excluir ya-proxy (tiene sufijo)
for %%A in ("%P%") do set "NAME=%%~nxA"
set "T=!NAME:%PROXY_SUFFIX%=!"
if /I not "!T!"=="!NAME!" goto :EOF

for %%A in ("%P%") do set "BASE=%%~nA"
set /a TOTAL+=1
call :ONE "%P%" "%BASE%"
goto :EOF
...
:WRITE_PS_ASS
REM Genera overlay.ass mostrando "N - NombreDelProxy" arriba/centro con ~80%% opacidad
...
>>"%~1" echo $lines += "PlayResX: 1080"
>>"%~1" echo $lines += "PlayResY: 608"
>>"%~1" echo $lines += "WrapStyle: 2"
>>"%~1" echo $lines += ""
...
>>"%~1" echo $lines += "Style: Top,Arial,18,&H33FFFFFF,&H00FFFFFF,&H66000000,&H00000000,0,0,0,0,100,100,0,0,1,1,0,8,10,10,8,1"
>>"%~1" echo $lines += ""
>>"%~1" echo $lines += "[Events]"
...
>>"%~1" echo ^    $dur = ^& $FFProbe -v error -show_entries format^=duration -of default^=nw^=1:nk^=1 "$p"
...
:END
if exist "%STATUS_LOG%" call :LOG "[END] ExitCode=%ERRORLEVEL%"
